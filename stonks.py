...
class Stonk:
    ...
    def get_market_data(self):
        ...
        self.date_series = pd.np.column_stack(df.index.values)
        self.close_series = pd.np.column_stack(df['Close'].values)
...
if __name__ == '__main__':
    ...
    """
     index = 0
     while True:
         if ord(self.ticker[index]) < ord(other.ticker[index]):
             return False
         elif ord(self.ticker[index]) == ord(other.ticker[index]):
             if min(len(self.ticker), len(other.ticker)) > index:
                 index += 1
             else:
                 return not len(self.ticker) < ord(other.ticker)
         else:
             return True


     word_length = min(len(self.ticker), len(other.ticker))

     for i in range(word_length):
         if ord(self.ticker[i]) < ord(other.ticker[i]):
             return False
         elif ord(self.ticker[i]) > ord(other.ticker[i]):
             return True

     return len(self.ticker) < len(other.ticker)
     """
